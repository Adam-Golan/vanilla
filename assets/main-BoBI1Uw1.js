...
const __vitePreload = /* @__PURE__ */ __name(function preload(baseModule, deps, importerUrl) {
  ...
  if (deps && deps.length > 0) {
    let allSettled2 = /* @__PURE__ */ __name(function(promises) {
      return Promise.all(
        promises.map(
...
const _Storage = class _Storage {
  constructor(db = "DB", store = "general") {
    __publicField(this, "storageTypes", ["localStorage", "sessionStorage", "indexedDB"]);
    this.db = db;
    this.store = store;
  }
  /**
...
const _Preference = class _Preference {
  constructor() {
    __publicField(this, "storage", new Storage());
    __publicField(this, "storageKey", _Preference.name.toLowerCase());
    __publicField(this, "initialFontSize");
...
const _Basis = class _Basis extends HTMLElement {
  ...
  constructor(config) {
     ...
     */
    __publicField(this, "idElem", /* @__PURE__ */ __name((id) => this.querySelector(`[id="${id}"]`), "idElem"));
    /**
...
const _Navigation = class _Navigation {
   ...
   */
  async importTexts() {
    await this.i18n.importTexts(await this.state.get(StateKeys.preferences).getLang());
    this.fisrtLoad();
  }
  /**
