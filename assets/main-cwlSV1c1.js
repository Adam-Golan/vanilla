  ...
  */
  ...
  navigationLogic(path) {
    ...
    if (this.cachedPages.has(path)) {
      ...
      if (Page2.name === this.currentPage?.constructor.name && !location.pathname.includes(path)) return;
...
const _Main = class _Main {
  ...
  subscribes() {
    const modals = {};
    this.appState.subscribe(StateKeys.openModal, (key, content) => modals[key] = new Modal(this.app.append.bind(this.app), content, () => delete modals[key]));
  }
};
__name(_Main, "Main");
let Main = _Main;
